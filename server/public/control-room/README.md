...
Desktop registrations collect a phone number and email for administrator review and are stored as `pending` until an administrator approves them from “用户审核”. No email or SMS verification is required. Approval grants 24 hours by default; the control room can save a custom duration in hours or set the account to permanent access. Expiration is enforced by login, `/api/auth/me`, and every authenticated desktop request, so an expired account cannot continue using an existing token. Revoking an account removes its existing sessions immediately; the desktop client then returns to its login screen on the next authenticated request.
