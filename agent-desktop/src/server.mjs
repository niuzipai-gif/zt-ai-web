...
const tasks = new AgentTaskManager({ workspaceRoot, permissionStore: permissions, gatewayUrl, historyPath: path.join(DATA, 'tasks.json') })
await tasks.load()
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/state') return json(request, response, 200, { ok: true, ...tasks.snapshot(), permissions: permissions.snapshot(), gatewayUrl, mode: 'execute' })
