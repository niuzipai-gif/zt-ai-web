...
网关默认运行在 `http://localhost:8790`，支持 MiniMax M3、DeepSeek V4 Flash，以及在聊天中隐式触发的 MMX 图片/视频请求。媒体请求统一由网关调用 MMX REST 接口，优先读取 `MMX_API_KEY`/`MMX_BASE_URL`，也兼容已有的 `MINIMAX_API_KEY`/`MINIMAX_BASE_URL`；浏览器和桌面端都不会接触密钥。公开网页会在打开或切换内部页面时发送轻量页面访问埋点；真正的聊天/媒体调用仍单独记录模型、估算 token、状态和消息。将网页部署到 GitHub Pages 后，GitHub Pages 只能托管静态前端；要让线上网页使用真实 API，还需要将 `server` 单独部署到 HTTPS 后端，并把该地址配置为构建时的 `VITE_API_BASE_URL`，同时在后端 `CORS_ORIGIN` 中加入 Pages 的 origin。
...
- `MINIMAX_API_KEY`
- `MMX_API_KEY`（图片/视频媒体服务；如果与文本服务共用密钥可省略，网关会回退到 `MINIMAX_API_KEY`）
- `DEEPSEEK_API_KEY`
