...
test('public voice mode can interrupt speaking and the mobile composer has a second input row', async () => {
  ...
  assert.doesNotMatch(source, /disabled=\{\['processing', 'speaking'\]\.includes\(state\.status\)\}/)
  assert.doesNotMatch(source, /\['processing', 'listening', 'speaking'\]\.includes\(state\.status\)/)
  assert.match(source, /disabled=\{greeting\.status === 'loading' \|\| \['processing'\]\.includes\(state\.status\)\}/g)
  assert.match(source, /voiceTurnRef/)
  assert.match(source, /if \(requestId !== voiceTurnRef\.current\) return/)
  assert.match(source, /playGreeting = async \(requestId = greetingRequestRef\.current\)/)
  assert.match(source, /if \(requestId !== greetingRequestRef\.current\) return result/)
