...
test('provides a matching resume file for every interface language', () => {
  for (const code of ['zh', 'en', 'ja']) {
    ...
    assert.equal(siteCopy[code].downloadCenter.platforms.length, 4)
    assert.match(siteCopy[code].downloadCenter.platforms.find(platform => platform.id === 'windows').url, /ZT\.buddy-Desktop-0\.2\.26-x64\.exe$/)
    assert.ok(siteCopy[code].downloadCenter.platforms.find(platform => platform.id === 'android').url)
    assert.equal(siteCopy[code].downloadCenter.platforms.find(platform => platform.id === 'apple').available, false)
    ...
    assert.ok(siteCopy[code].chat.sourcesTitle)
...
test('resume download paths point to published DOCX files', () => {
  assert.equal(resumeDocumentByLanguage.zh.name, '蔡宙廷_FDE个人简历.docx')
  for (const code of ['zh', 'en', 'ja']) {
